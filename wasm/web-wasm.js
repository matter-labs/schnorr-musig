...
const wasmFile = './dist/schnorr-musig-web_bg.wasm';
const jsFile   = './dist/schnorr-musig-web.js';
...
export async function loadSchnorrMusig(wasmFileUrl) {
  if (!wasmFileUrl) {
    const wasmResponse = new Response(wasmBytes, wasmResponseInit);
    await init(wasmResponse);
  } else {
    await init(DefaultSchnorrMusigWasmURL);
  }
}
`;
