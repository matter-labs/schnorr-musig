...
use franklin_crypto::alt_babyjubjub::{AltJubjubBn256, fs::Fs, fs::FsRepr};
...
impl Decoder{
    ...
    pub fn decode_pubkey_list(
        input: &[u8]
    )-> Result<Vec<PublicKey<Bn256>>, MusigABIError>{
    ...
    pub fn decode_commitments(
        input: &[u8]
    ) -> Result<Vec<Point<Bn256, Unknown>>, MusigABIError>{
...
#[test]
    fn test_pubkey_list_and_privkey_decoder(){
        ...
        for i in 0..number_of_keys{
            ...
            let pubkey = PublicKey::from_private(
                &privkey, 
                generator, 
                &params
            );
            ...
            pubkey.write(&mut encoded_pubkeys[offset..(offset + length_of_encoding_data)]).unwrap();
        ...
        for i in 0..number_of_keys{
            ...
            expected_privkeys[i].0.into_repr().write_be(&mut encoded_privkey[..]).unwrap();
