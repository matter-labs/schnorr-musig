...
fn musig_wasm_multiparty_full_round() {
    ...
    for (position, wasm_signer) in wasm_signers.iter_mut().enumerate() {
        ...
        privkeys[position]
            ...
            .write_be(&mut encoded_privkey[..])
            .unwrap();
        let result = wasm_signer.sign(&encoded_privkey, &message).unwrap();
        challenge = result[STANDARD_ENCODING_LENGTH..].to_vec();
        signature_shares.extend_from_slice(&result[..STANDARD_ENCODING_LENGTH]);
    }
    ...
    for wasm_signer in wasm_signers.iter_mut() {
        let agg_sig = wasm_signer
            .receive_signature_shares(&signature_shares, &challenge)
            .unwrap();
    ...
    for position in (0..pubkeys.len()).skip(1) {
        ...
        let is_verified = MusigBN256WasmVerifier::verify(&message, &encoded_pubkeys, &sig).unwrap();
        assert!(is_verified);
    }
}
